...
function generateReceiptHTML(pkg: ReceiptPkg) {
  ...
  const row = (label: string, value: unknown) =>
    value == null || value === '' ? '' : `<p style="margin:2px 0">${esc(label)}: <b>${esc(value)}</b></p>`;

  return `
    <div style="font-family:monospace;font-size:11px;width:380px;color:#000;background:#fff;padding:16px">
      <div style="text-align:center;border-bottom:1px solid #000;padding-bottom:8px;margin-bottom:8px">
        <h1 style="font-size:20px;font-weight:bold;letter-spacing:2px;margin:0">SWIFTDROP</h1>
        <p style="margin:2px 0">Fast &amp; Reliable Delivery</p>
        <p style="margin:2px 0">TEL: +254114606020</p>
      </div>
      ${row('PARCEL NO', pkg.trackingNumber)}
      ${row('SENDER', pkg.senderName)}
      ${row('FROM', pkg.senderAddress)}
      ...
      ${row('M-PESA REF', pkg.mpesaReceiptNumber)}
      <div style="border:1px solid #000;padding:6px;margin:8px 0;font-size:9px">
